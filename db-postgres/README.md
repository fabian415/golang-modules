...
這是一個基於 Wails v2 框架的 PostgreSQL 操作範例專案，展示了如何在前端 Vue.js 應用中直接呼叫後端 Golang 函式來操作 PostgreSQL 資料庫。專案採用現代化的架構設計，包含資料庫遷移、環境變數管理、跨平台支援等企業級功能。
...
確保 PostgreSQL 服務正在運行，並創建對應的資料庫：
...
| `DB_PORT` | 資料庫連接埠 | 5432 | 否 |
| `DB_USER` | 資料庫使用者名稱 | postgres | 否 |
| `DB_PASSWORD` | 資料庫密碼 | - | 是 |
| `DB_NAME` | 資料庫名稱 | postgres | 否 |
| `DB_SSLMODE` | SSL 模式 | disable | 否 |

**SSL 模式選項：**
- `disable` - 不使用 SSL（開發環境推薦）
- `require` - 需要 SSL
- `verify-ca` - 驗證 CA 憑證
- `verify-full` - 完整驗證（生產環境推薦）
